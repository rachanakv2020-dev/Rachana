...
router.get("/", requireAdmin, async (_req, res) => {
  try {
    const { rows: payments } = await pool.query(
      `SELECT p.id, p.order_id AS "orderId", u.name AS customer, p.amount, p.method, p.status, p.created_at AS "createdAt"
...
router.patch("/:id/status", requireAdmin, async (req, res) => {
  try {
    ...
    const result = await pool.query("UPDATE payments SET status = $1 WHERE id = $2", [req.body.status, req.params.id]);
